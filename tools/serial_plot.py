...
class serial_plotter_class:
    ...
    def set_figure(self, message_id):
        if(message_id == 0):
            ...
            plt.ylabel('ICL CoG')
            plt.ylim([-180, 180])
            self.create_curve('Wx', 'blue')
            self.create_curve('Wy', 'orange')
            self.create_curve('Wz', 'green')
            plt.subplot(313)
            plt.ylabel('ICL CoG')
            plt.ylim([-1, 1])
            self.create_curve('thetaX', 'pink')
            self.create_curve('thetaY', 'gray')
            self.show_subplot()
    ...
    def serial_receive(self):
        ...
        if self.plot_begin == False:
            ...
            self.serial_data = [serial_data_class(
                200) for i in range(0, self.curve_number)]
