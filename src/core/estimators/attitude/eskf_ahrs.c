...
MAT_ALLOC(P, 3, 3);             //process covariance matrix of error state
...
void eskf_ahrs_init(float dt)
{
	...
	/* initialize P matrix */
	mat_data(P)[0*3 + 0] = 5.0f;
	mat_data(P)[0*3 + 1] = 0.0f;
	mat_data(P)[0*3 + 2] = 0.0f;

	mat_data(P)[1*3 + 0] = 0.0f;
	mat_data(P)[1*3 + 1] = 5.0f;
	mat_data(P)[1*3 + 2] = 0.0f;

	mat_data(P)[2*3 + 0] = 0.0f;
	mat_data(P)[2*3 + 1] = 0.0f;
	mat_data(P)[2*3 + 2] = 5.0f;
...
void eskf_ahrs_accelerometer_correct(float *accel)
{
	...
	MAT_MULT(&P, &H_accel_t, &PHt_accel);
	MAT_MULT(&H_accel, &PHt_accel, &HPH_t_accel);
	MAT_ADD(&HPH_t_accel, &V_accel, &HPHt_V_accel);
	...
	mat_data(KH_accel)[0*3 + 0] = 1.0f - mat_data(KH_accel)[0*3 + 0];
	mat_data(KH_accel)[2*3 + 1] = 1.0f - mat_data(KH_accel)[2*3 + 1];
	mat_data(KH_accel)[2*3 + 2] = 1.0f - mat_data(KH_accel)[2*3 + 2];
