...
enum ENUM_MAV_CMDS {
	ENUM_HANDLER_FUNC(mav_request_autopilot_capabilities),
...
struct mavlink_parser_item cmd_list[] = {
	MAV_CMD_DEF(mav_request_autopilot_capabilities, 520),
	/* mission commands */
	...
	//MAV_CMD_DEF(mav_mission_command, 76),
...
void parse_mavlink_received_msg(mavlink_message_t *msg)
{
	...
	for(i = 0; i < (signed int)CMD_LEN(cmd_list); i++) {
		if(msg->msgid == cmd_list[i].msg_id) {
			cmd_list[i].handler();
			msg->msgid = 0; //reset mavlink msg id
			break;
		}
	}
}
